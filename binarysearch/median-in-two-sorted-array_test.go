...
func TestFindMediaInTwoSortedArrayII(t *testing.T) {
	type args struct {
		nums1 []int
		nums2 []int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := FindMediaInTwoSortedArrayII(tt.args.nums1, tt.args.nums2); got != tt.want {
				t.Errorf("FindMediaInTwoSortedArrayII() = %v, want %v", got, tt.want)
			}
		})
	}
}
