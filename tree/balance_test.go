...
func TestIsBalancedTreeByIteration(t *testing.T) {
	type args struct {
		t *Tree
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IsBalancedTreeByIteration(tt.args.t.root); got != tt.want {
				t.Errorf("IsBalancedTreeByIteration() = %v, want %v", got, tt.want)
			}
		})
	}
}
