...
func TestSearchWordInBoardByBFSII(t *testing.T) {
	type args struct {
		board [][]byte
		word  string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := SearchWordInBoardByBFSII(tt.args.board, tt.args.word); got != tt.want {
				t.Errorf("SearchWordInBoardByBFSII() = %v, want %v", got, tt.want)
			}
		})
	}
}
